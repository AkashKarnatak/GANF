...
import numpy as np
from dataset import load_drone
...
train_loader, val_loader, test_loader, n_attr = load_drone(
    args.data_dir, args.batch_size, 30
)
...
if args.graph != "None":
    ...
    init = torch.zeros([n_attr, n_attr])
    init = xavier_uniform_(init).abs()
    init = init.fill_diagonal_(0.0)
A = init.clone().detach().to(device).requires_grad_(True)
    ...
    while rho < rho_max:
        ...
        for _ in range(args.n_epochs):
            ...
            for x in train_loader:
                ...
                loss = -model(x, A_hat)
                h = torch.trace(torch.matrix_exp(A_hat * A_hat)) - n_attr
            ...
            print(
                "Epoch: {}, train -log_prob: {:.2f}, test -log_prob: {:.2f}, h: {}".format(
                    epoch, np.mean(loss_train), np.mean(loss_val), h.item()
                )
            )
            ...
            if np.mean(loss_val) < loss_best:
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(save_path, "{}_best.pt".format(args.name)),
                )
        ...
        torch.save(
            model.state_dict(),
            os.path.join(save_path, "{}_{}.pt".format(args.name, epoch)),
        )
...
for _ in range(100):
    ...
    if np.mean(loss_val) < loss_best:
        ...
        torch.save(A.data, os.path.join(save_path, "graph_best.pt"))
        torch.save(
            model.state_dict(), os.path.join(save_path, "{}_best.pt".format(args.name))
        )
